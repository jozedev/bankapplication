...
import com.jozedev.bankapplication.backend.account.service.AccountService;
import com.jozedev.bankapplication.backend.account.service.ServiceClient;
import com.jozedev.bankapplication.backend.account.service.TransactionService;
import com.jozedev.bankapplication.backend.account.service.TransactionServiceImpl;
import com.jozedev.bankapplication.backend.account.util.ServiceKeys;
...
public class TransactionServiceTests {
    ...
    void testGetAllByAccountClientIdAndDateBetween() {
        ...
        client.setName("Armando Broja");

        when(serviceClient.getForEntity(ServiceKeys.CLIENT_INFO_SERVICE, ClientDto.class, 1L))
                .thenReturn(new ResponseEntity<>(client, HttpStatus.OK));

        List<BankStatementDto> listExpected = new ArrayList<>(List.of(
                new BankStatementDto(LocalDateTime.of(2021, 1, 1, 10, 0, 0), client.getName(), "number", "savings", 100, true, "deposit", 50, 100),
                new BankStatementDto(LocalDateTime.of(2021, 1, 2, 10, 0, 0), client.getName(), "number", "savings", 100, true, "payment", -10, 100),
                new BankStatementDto(LocalDateTime.of(2021, 1, 3, 10, 0, 0), client.getName(), "number", "savings", 100, true, "deposit", 20, 100),
                new BankStatementDto(LocalDateTime.of(2021, 1, 4, 10, 0, 0), client.getName(), "number", "savings", 100, true, "payment", -20, 100)));
